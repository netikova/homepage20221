...
public class FileMngUtil {
	...
	public List<FileVO> parseFileInf(Map<String, MultipartFile> files, String KeyStr,
		...
		if(!saveFolder.exists() || saveFolder.isFile()) {
			saveFolder.mkdirs();      //파일 경로 생성
		}
		
		
		//파일변수 iterator = java.utill
		...
		while(itr.hasNext()) {
			...
			//파일확장자 체크
			int index = orginFileName.lastIndexOf(".");   //.이후의 수를 찾는다.
			String fileExt = orginFileName.substring(index + 1);
			
			//저장파일명(원본파일명, 저장파일명(1),(2))        
			...
			//파일저장
			if(!"".equals(orginFileName)) {  //separator가 없으면 최근폴더와 같은 위치에 저장된다.
